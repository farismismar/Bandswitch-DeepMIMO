...
##############################################################################
df_legacy = df.copy()
df_legacy['y'] = pd.DataFrame((df_legacy.loc[:,'Capacity_35'] <= rate_threshold) & (df_legacy.loc[:,'Capacity_28'] >= rate_threshold), dtype=int)
...
df_blind['y'] = pd.DataFrame((df_blind.loc[:,'Capacity_35'] <= rate_threshold), dtype=int)
...
train, benchmark_data_proposed = train_test_split(df, test_size=r_exploitation, random_state=seed)
...
#######
[y_pred, y_score, clf] = train_classifier(train, r_training=max_r_training)
y_pred_proposed = predict_handover(benchmark_data_proposed, clf)
y_score_proposed = clf.predict_proba(benchmark_data_proposed.drop(['y'], axis=1))
y_test_proposed = benchmark_data_proposed['y']
...
benchmark_data_proposed.loc[C0, 'Capacity_Proposed'] = benchmark_data_proposed['Capacity_35']

# Class-1
C1 = y_pred_proposed.iloc[:,0] == 1
benchmark_data_proposed.loc[C1, 'Capacity_Proposed'] = benchmark_data_proposed['Capacity_28']
